...
public class LoginService : ILoginService, ITransientDependency
{
    ...
    {
        var token = await _storage.GetAsync(Alfa5Consts.OidcConsts.AccessTokenKeyName);
    ...
    {
        var refreshToken = await _storage.GetAsync(Alfa5Consts.OidcConsts.RefreshTokenKeyName);
    ...
    {
        await _storage.SetAsync(Alfa5Consts.OidcConsts.AccessTokenKeyName, accessToken);
        await _storage.SetAsync(Alfa5Consts.OidcConsts.RefreshTokenKeyName, refreshToken);
    }

    private async Task ClearTokenCacheAsync()
    {
        await _storage.RemoveAsync(Alfa5Consts.OidcConsts.AccessTokenKeyName);
        await _storage.RemoveAsync(Alfa5Consts.OidcConsts.RefreshTokenKeyName);
    }
}