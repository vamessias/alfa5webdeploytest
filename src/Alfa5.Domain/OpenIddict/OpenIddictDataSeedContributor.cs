...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("Alfa5") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "Alfa5", DisplayName = "Alfa5 API", Resources = { "Alfa5" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        var consoleAndAngularClientId = configurationSection["Alfa5_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["Alfa5_App:RootUrl"]?.TrimEnd('/');
        ...
        //MAUI Client
        var mauiClientId = configurationSection["Alfa5_Maui:ClientId"];
        if (!mauiClientId.IsNullOrWhiteSpace())
        {
            var mauiClientRootUrl = configurationSection["Alfa5_Maui:RootUrl"]?.Replace("_", "-");
            var mauiClientPostLogoutRedirectUri = configurationSection["Alfa5_Maui:PostLogoutRedirectUri"]?.Replace("_", "-");
            await CreateApplicationAsync(
                name: mauiClientId!,
                type: OpenIddictConstants.ClientTypes.Public,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
        ...
        var blazorWebAppClientId = configurationSection["Alfa5_BlazorWebApp:ClientId"];
        if (!blazorWebAppClientId.IsNullOrWhiteSpace())
        {
            var blazorWebAppRootUrl = configurationSection["Alfa5_BlazorWebApp:RootUrl"]!.EnsureEndsWith('/');

            await CreateApplicationAsync(
                name: blazorWebAppClientId!,
                type: OpenIddictConstants.ClientTypes.Confidential,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
                displayName: "Blazor WebApp Application",
                secret: configurationSection["Alfa5_BlazorWebApp:ClientSecret"] ?? "1q2w3e*",
                grantTypes: new List<string> //Hybrid flow
                {
                    OpenIddictConstants.GrantTypes.AuthorizationCode, OpenIddictConstants.GrantTypes.Implicit
                },
                scopes: commonScopes,
                redirectUri: $"{blazorWebAppRootUrl}signin-oidc",
                postLogoutRedirectUri: $"{blazorWebAppRootUrl}signout-callback-oidc",
                clientUri: blazorWebAppRootUrl,
                logoUri: "/images/clients/blazor.svg"
            );
        }

        // Swagger Client
        var swaggerClientId = configurationSection["Alfa5_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["Alfa5_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        var webPublicTieredClientId = configurationSection["Alfa5_Web_Public:ClientId"];
        if (!webPublicTieredClientId.IsNullOrWhiteSpace())
        {
            var webPublicTieredRootUrl = configurationSection["Alfa5_Web_Public:RootUrl"]!.EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["Alfa5_Web_Public:ClientSecret"] ?? "1q2w3e*",
                grantTypes: new List<string> //Hybrid flow
                {
                    OpenIddictConstants.GrantTypes.AuthorizationCode, OpenIddictConstants.GrantTypes.Implicit
                },
