...
````bash
Add-Migration Your_Migration_Name -Context Alfa5DbContext
````

Using EF Core command line tool;

````bash
dotnet ef migrations add Your_Migration_Name --context Alfa5DbContext
````
...
````bash
Add-Migration Your_Migration_Name -Context Alfa5TenantDbContext -OutputDir TenantMigrations
````

Using EF Core command line tool:

````bash
dotnet ef migrations add Your_Migration_Name --context Alfa5TenantDbContext --output-dir TenantMigrations
````
